...
class App:
    def __init__(self, root):
        ...
        self.img_rect=(0,0,1,1); self.running=True; self.build_ui(); self.bind_inputs(); root.after(30,self.ui_tick)
    ...
    def build_ui(self):
        ...
        self.canvas.bind("<Button-1>",self.on_canvas_left); self.canvas.bind("<Button-3>",self.on_canvas_right)
        ttk.Label(right,text="Key Mapping",font=("Segoe UI",14,"bold")).pack(anchor="w",pady=(0,8))
        ttk.Label(right,text="Батырманы бас → экрандағы дәл орнына бас.",wraplength=280).pack(anchor="w",pady=(0,8))
        ...
        ttk.Label(right,text="W/A/S/D — жүру\nShift — жүгіру\nSpace — секіру\nR — оқтау\nF — әрекет\nMouse Left — экранды басу\nMouse Right — прицел",justify="left").pack(anchor="w")
    ...
    def bind_inputs(self): self.root.bind_all("<KeyPress>",self.key_down); self.root.bind_all("<KeyRelease>",self.key_up)
    ...
    def on_canvas_left(self,e):
        ...
        if not n:return
        if self.bind_mode:
            self.profile[self.bind_mode]=[n[0],n[1]]; self.status.set(f"{LABELS[self.bind_mode]} қойылды"); self.bind_mode=None; self.redraw(); return
        self.send(f"TAP {n[0]:.5f} {n[1]:.5f}")
    def on_canvas_right(self,e):
        if self.bind_mode:return
    ...
    def movement_loop(self):
        while self.running:
            dx=(1 if "d" in self.pressed else 0)-(1 if "a" in self.pressed else 0); dy=(1 if "s" in self.pressed else 0)-(1 if "w" in self.pressed else 0)
            if (dx or dy) and self.connected:
                mag=math.hypot(dx,dy); dx/=mag; dy/=mag; c=self.profile.get("joystick",[0.18,0.73]); r=float(self.profile.get("joystick_radius",0.10))
                x2=max(0,min(1,c[0]+dx*r)); y2=max(0,min(1,c[1]+dy*r)); self.send(f"SWIPE {c[0]:.5f} {c[1]:.5f} {x2:.5f} {y2:.5f} 160"); time.sleep(0.145)
    ...
    def connect_worker(self,ip,pin):
        try:
            ...
            self.video_sock,self.ctrl_sock=vs,cs; self.connected=True; self.root.after(0,lambda:(self.status.set("Қосылды"),self.btn_connect.config(text="Ажырату"))); self.video_loop(vs)
    ...
    def redraw(self):
        ...
        rw,rh=max(1,int(iw*scale)),max(1,int(ih*scale)); x=(cw-rw)//2; y=(ch-rh)//2; disp=self.current_image.resize((rw,rh),Image.Resampling.BILINEAR)
        ...
        if c:
            r=float(self.profile.get("joystick_radius",0.10))*min(rw,rh); px=x+c[0]*rw; py=y+c[1]*rh; self.canvas.create_oval(px-r,py-r,px+r,py+r,outline="white",dash=(5,3))
